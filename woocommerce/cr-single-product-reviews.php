...
<div id="reviews" class="woocommerce-Reviews">
	...
	/*		
	echo '5 star' . $product->get_rating_count( 5 ) . '<br>';
	echo '4 star' . $product->get_rating_count( 4 ) . '<br>';
	echo '3 star' . $product->get_rating_count( 3 ) . '<br>';
	echo '2 star' . $product->get_rating_count( 2 ) . '<br>';
	echo '1 star' . $product->get_rating_count( 1 ) . '<br>';
	*/
			if ( $count && wc_review_ratings_enabled() ) {
				/* translators: 1: reviews count 2: product name */
				$reviews_title = sprintf( esc_html( _n( '%1$s review for %2$s', '%1$s reviews for %2$s', $count, 'woocommerce' ) ), esc_html( $count ), '<span>' . get_the_title() . '</span><br><br>' );
		...
		<?php if ( have_comments() ) : ?>
			...
			<ol class="commentlist">
				<?php 
				wp_list_comments( apply_filters( 'woocommerce_product_review_list_args', array( 'callback' => 'rjs_comments_walker' ) ) ); 
	...
	<?php if ( get_option( 'woocommerce_review_rating_verification_required' ) === 'no' || wc_customer_bought_product( '', get_current_user_id(), $product->get_id() ) ) : ?>
		<div id="review_form_wrapper">
			<div id="review_form" class="cr-single-product-review">
				...
				$comment_form['comment_field'] .= '<p class="comment-form-comment"><label for="comment">' . esc_html__( 'Your review', 'woocommerce' ) . '&nbsp;<span class="required">*</span></label><textarea id="comment" name="comment" placeholder="Your Review" cols="45" rows="8" required class="cr-review-form-textbox"></textarea></p>';
